...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules2: {
          ...
          currency:[
            {validator:checkbanlance,trigger:"change"}
          ]
        }
    };
  },
  ...
  computed:{
      IsHanBanlances(){
        ...
        var array=this.BanlanceArray.map(function(item){return item});
        var XAS_Index=-1;
        ...
        if(this.form.currency=="XAS"){
          if(amount>array[XAS_Index].balance-0.1){
            return {success:false,error:this.form.currency+gt+(array[CY_index].balance-0.1)}
          }
          return {success:true}
        }else if(this.form.currency==Config.cy){
          if(amount>(array[CY_index].balance)){
            return {success:false,error:this.form.currency+gt+(array[CY_index].balance)}
          }
          return {success:true}
        ...
        .then(data => {
          ...
          if (data.success) {
            this.TransationArray = data.transfers.map(item => {
              item.amounts = item.amount / 1e8 + " " + item.currency;
    ...
    TiXian(number,currency){
     ...
     // var fee=String(0);
      ...
      .then(data=>{
        ...
        if(data.success){
          this.$message.success(this.$t("message.success_tixian")+"["+number+" "+currency+"]");
          this.dialogFormVisible=false;
          this.begin_timer();
        }else{
